...
class WindowsDeviceRegistry(DeviceRegistryBase):
    ...
    def create_device(self, device_path: str, is_composite: bool, hub_handle: HANDLE, usb_port_num: int) -> Device:
        ...
        if delta > 1.0:
            logging.warning(f'Getting string descriptors took exceptionally long ({delta:.1f}s); the device might not be properly working (vid=0x{device.vid:04X}, pid=0x{device.pid:04X}, manufacturer={device.manufacturer}, product={device.product}, serial={device.serial}, path={device_path})')
